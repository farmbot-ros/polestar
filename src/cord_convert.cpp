...
class CoordinateTransformer : public rclcpp::Node {
  ...
  private:
    void ecef2enuCallback(const std::shared_ptr<farmbot_interfaces::srv::Ecef2Enu::Request> request,
        ...
        for (const auto &pose : request->ecef) {
            ...
            std::tuple<double, double, double> ecef_to_enu =
                concord::ecef_to_enu(std::make_tuple(pose.position.x, pose.position.y, pose.position.z),
    ...
    void ecef2gpsCallback(const std::shared_ptr<farmbot_interfaces::srv::Ecef2Gps::Request> request,
        ...
        for (const auto &pose : request->ecef) {
            ...
            std::tuple<double, double, double> ecef_to_gps =
                concord::ecef_to_gps(pose.position.x, pose.position.y, pose.position.z);
    ...
    void enu2ecefCallback(const std::shared_ptr<farmbot_interfaces::srv::Enu2Ecef::Request> request,
        ...
        for (const auto &pose : request->enu) {
            ...
            std::tuple<double, double, double> enu_to_ecef =
                concord::enu_to_ecef(std::make_tuple(pose.position.x, pose.position.y, pose.position.z),
                                     std::make_tuple(geo_datum.latitude, geo_datum.longitude, geo_datum.altitude));
    ...
    void enu2gpsCallback(const std::shared_ptr<farmbot_interfaces::srv::Enu2Gps::Request> request,
        ...
        for (const auto &pose : request->enu) {
            sensor_msgs::msg::NavSatFix gps;
            std::tuple<double, double, double> enu_to_gps =
                concord::enu_to_gps(pose.position.x, pose.position.y, pose.position.z, geo_datum.latitude,
                                    geo_datum.longitude, geo_datum.altitude);
    ...
    void gps2ecefCallback(const std::shared_ptr<farmbot_interfaces::srv::Gps2Ecef::Request> request,
        ...
        for (const auto &gps : request->gps) {
            geometry_msgs::msg::Pose ecef_pose;
            std::tuple<double, double, double> gps_to_ecef =
                concord::gps_to_ecef(gps.latitude, gps.longitude, gps.altitude);
    ...
    void gps2enuCallback(const std::shared_ptr<farmbot_interfaces::srv::Gps2Enu::Request> request,
        ...
        for (const auto &gps : request->gps) {
            geometry_msgs::msg::Pose enu_pose;
            std::tuple<double, double, double> gps_to_enu = concord::gps_to_enu(
                gps.latitude, gps.longitude, gps.altitude, geo_datum.latitude, geo_datum.longitude, geo_datum.altitude);
