...
class TransformPub : public rclcpp::Node {
    ...
    public:
        TransformPub() : Node(
            ...
            name = this->get_parameter_or<std::string>("name", "transform_pub");
    ...
    private:
        void ecef_callback(const nav_msgs::msg::Odometry::ConstSharedPtr& odom) {
            ...
            RCLCPP_INFO_ONCE(this->get_logger(), "TRANSFORMS FOR <%s> SET", namespace_.c_str());
            ecef_sub_.reset();
        }
        ...
        void base_transform(const nav_msgs::msg::Odometry::ConstSharedPtr& odom) {
            ...
            // Send identity transform from /odom to /base_link
            ...
            // Send odometry transform from /map to /odom
            odom_transform();
        }

        void odom_transform() {
            geometry_msgs::msg::TransformStamped stat_t;
            stat_t.header.stamp = odom_msg.header.stamp;
            stat_t.header.frame_id = namespace_ + "/map";
            stat_t.child_frame_id = namespace_ + "/odom";
            stat_t.transform.translation.x = odom_msg.pose.pose.position.x;
            stat_t.transform.translation.y = odom_msg.pose.pose.position.y;
            stat_t.transform.translation.z = odom_msg.pose.pose.position.z;
            if (!altitude) {
                stat_t.transform.translation.z = 0.0;
            }
            stat_t.transform.rotation.x = odom_msg.pose.pose.orientation.x;
            stat_t.transform.rotation.y = odom_msg.pose.pose.orientation.y;
            stat_t.transform.rotation.z = odom_msg.pose.pose.orientation.z;
            stat_t.transform.rotation.w = odom_msg.pose.pose.orientation.w;
