...
class TransformPub {
  private:
    ...
    rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr world_to_map_, foot_to_base_, odom_to_foot_,
        mapt_to_odom_;
  ...
  public:
    TransformPub(rclcpp::Node::SharedPtr node) : node_(node), updater_(node) {
        ...
        world_to_map_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            "ref/ecf", 10, [this](const geometry_msgs::msg::PoseStamped::ConstSharedPtr &ecef) { world_2_map(ecef); });
        mapt_to_odom_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            "loc/enu", 10, [this](const geometry_msgs::msg::PoseStamped::ConstSharedPtr &odom) { map_2_odom(odom); });
        odom_to_foot_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            "loc/enu", 10, [this](const geometry_msgs::msg::PoseStamped::ConstSharedPtr &odom) { odom_2_foot(odom); });
        foot_to_base_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            "loc/enu", 10, [this](const geometry_msgs::msg::PoseStamped::ConstSharedPtr &odom) { foot_2_base(odom); });
    }
  ...
  private:
    ...
    void world_2_map(const geometry_msgs::msg::PoseStamped::ConstSharedPtr &ecef) {
        auto x = world_ref_ == "datum" ? 0.0 : ecef->pose.position.x;
        auto y = world_ref_ == "datum" ? 0.0 : ecef->pose.position.y;
    ...
    void map_2_odom(const geometry_msgs::msg::PoseStamped::ConstSharedPtr &odom) {
        geometry_msgs::msg::TransformStamped stat_t;
        stat_t.header.stamp = node_->get_clock()->now();
    ...
    void odom_2_foot(const geometry_msgs::msg::PoseStamped::ConstSharedPtr &odom) {
        ...
        dyna_t.transform.translation.x = odom->pose.position.x;
        dyna_t.transform.translation.y = odom->pose.position.y;
        dyna_t.transform.translation.z = 0.0;
        dyna_t.transform.rotation.x = odom->pose.orientation.x;
        dyna_t.transform.rotation.y = odom->pose.orientation.y;
        dyna_t.transform.rotation.z = odom->pose.orientation.z;
        dyna_t.transform.rotation.w = odom->pose.orientation.w;
        base_tf->sendTransform(dyna_t);
        basef_to_basel = true;
    }

    void foot_2_base(const geometry_msgs::msg::PoseStamped::ConstSharedPtr &odom) {
        geometry_msgs::msg::TransformStamped dyna_t;
        ...
        dyna_t.transform.translation.z = odom->pose.position.z;
        if (!altitude) {
            dyna_t.transform.translation.z = 0.0;
        }
